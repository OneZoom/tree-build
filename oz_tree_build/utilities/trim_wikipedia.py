...
def process_all_lines(wikipedia_file, output_stream, language="en"):
    ...
    for line_num, line in enumerate_lines_from_file(wikipedia_file):
        if line_num > 0 and line_num % 100000 == 0:
            logging.info(
                f"Kept {preserved_lines} out of {line_num-1} processed lines ({preserved_lines / line_num * 100:.2f}%))"
            )
    ...
    logging.info(
        f"Writing {len(vernacular_lines)} vernacular lines at the end of the file"
    )
