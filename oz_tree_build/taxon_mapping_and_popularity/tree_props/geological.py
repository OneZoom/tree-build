...
def prop_geological(tree):
    """
    Given a DendroPy tree object, add a "geological" attribute to each node,
    representing a 1-based period index.

    Assumes the tree already has a "date" attribute representing an absolute age in Mya.
    ...
    for node in tree.preorder_node_iter():
        n_age = getattr(node, "age", None)
        if n_age is None:
            logger.warning(f"Node {node.label} has no age property")
            node.geological = 0
        else:
            for mya_start, idx in lookup:  # noqa: B007  # idx is used outsid the lookup, not inside
                if n_age <= mya_start:
                    break
            else:
                # Fell off end
    ...
    tree.seed_node.prop_format["geological"] = "u8"
