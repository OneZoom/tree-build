    ...
    with open_file_based_on_extension(identifiers_filename, "rt") as identifiers_file:
        ...
        for EOLrow in reader:
            if reader.line_num % 1000000 == 0:
                logging.info(
                    "... {} rows read, {} used,  mem usage {:.1f} Mb".format(
...
def identify_best_EoLdata(OTT_ptrs, sources):
    ...
    for OTTid, data in OTT_ptrs.items():
        ...
        if len(choose) == 0:
            ...
            if len(choose) > 1:
                ...
                errstr = "More than one EoL ID {} for taxon OTT: {}".format(
                    choose, OTTid
                )
                dups += 1
    ...
    logging.info(
        " ✔ Of {} OpenTree taxa, {} ({:.2f}%) have EoL entries in the EoL identifiers file, and {} have multiple possible EOL ids. Mem usage {:.1f} Mb".format(
            validOTTs,
            OTTs_with_EOLmatch,
            OTTs_with_EOLmatch / validOTTs * 100,
            dups,
...
def write_popularity_tree(tree, outdir, filename, version, verbosity=0):
    ...
    with open(
        os.path.join(outdir, "{}_{}.nwk".format(filename, version)), "w+"
