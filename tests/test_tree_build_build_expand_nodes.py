"""
Tests for oz_tree_build.tree_build.build_oz_tree:expand_nodes
"""
...
from oz_tree_build.tree_build.build_oz_tree import NWK_READ_PARSER, expand_nodes


def do_expand_nodes(nwks, node_ages=None):
    ...
    with tempfile.TemporaryDirectory(prefix="ut_expand_nodes") as tmp_path:
        ...
        def mock_parse_one_zoom_token(node_label, parts_folders=None):
        ...
        with unittest.mock.patch(
            "oz_tree_build.tree_build.build_oz_tree.parse_one_zoom_token", mock_parse_one_zoom_token
        ):
            t = ete4.Tree(oz_token_vals["base@"]["file"], parser=NWK_READ_PARSER)
...
def test_missing_file(caplog):
    """When a referenced subtree file is missing, strip @ from node name and keep original children"""
    with tempfile.TemporaryDirectory(prefix="ut_expand_nodes") as tmp_path:
        base_path = os.path.join(tmp_path, "base.phy")
        with open(base_path, "w") as f:
            f.write("(((daisy:43,bessie:27)cows@,(george:23,wilma:9)pigs)field,(cat)farmhouse)farmyard;")

        missing_path = os.path.join(tmp_path, "cows.phy")  # deliberately not created

        def mock_parse_one_zoom_token(node_label, parts_folders=None):
            if node_label == "cows@":
                return dict(
                    file=missing_path,
                    node_name_in_parent="cows",
                    base_ott=None,
                    expand_nodes=False,
                    excluded_otts=[],
                )
        ...
        with unittest.mock.patch(
            ...
            with caplog.at_level(logging.WARNING, logger="oz_tree_build.tree_build.build_oz_tree"):
                t = ete4.Tree(base_path, parser=NWK_READ_PARSER)
                expand_nodes(t, {}, {})
                out = {t.root.name: node_to_dict(t.root)}

    assert any("cows.phy" in m and "not exist" in m for m in caplog.messages)
    assert out == {
        "farmyard": {
            "farmhouse": {"cat": None},
            "field": {
                # @ stripped, original children (daisy, bessie) retained from base tree
                "cows": {"bessie": None, "daisy": None},
                "pigs": {"george": None, "wilma": None},
            },
        }
    }
